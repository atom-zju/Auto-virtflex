...
template<class T>
int floor_idx_of_sampleq(deque<pair<long long, T>>& samples, int lo, int hi, long long timestamp){
	if( hi < lo || samples.empty() || hi >= samples.size()|| timestamp < samples[lo].first)
		return -1;
